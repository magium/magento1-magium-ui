...
class Magium_Clairvoyant_Magiumui_ConfigurationController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function saveOptions(array $options, $storeId = 0)
    {
        ...
        foreach ($collection as $item) {
            /* @var $item Magium_Clairvoyant_Model_Option */
        ...
        foreach ($options as $name => $value) {
            $configuration = Mage::getModel('magium_clairvoyant/option');
            /* @var $configuration Magium_Clairvoyant_Model_Option */
    ...
    {
        $testCase = Mage::helper('magium_clairvoyant')->getNullTestCase();
        ...
        foreach ($collection as $introspected) {
            /* @var $introspected Magium_Clairvoyant_Model_Introspected */
        ...
        foreach ($result as $component) {
            if ($component instanceof \Magium\Introspection\ComponentClass) {
                $introspected = Mage::getModel('magium_clairvoyant/introspected');
    ...
    protected function saveConfigurationItems($class, $items, $storeId)
    {
        ...
        $collection->getResource()->beginTransaction();
        foreach ($collection as $item) {
            /* @var $item Magium_Clairvoyant_Model_Configuration */
            if ($item->getStoreId() == $storeId) {
                $name = $item->getName();
        ...
        foreach ($items as $name => $value) {
            $configuration = Mage::getModel('magium_clairvoyant/configuration');
            /* @var $configuration Magium_Clairvoyant_Model_Configuration */
            $configuration->setStoreId($storeId);
            $configuration->setName($name);
